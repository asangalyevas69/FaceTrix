...
async def check_attendance(message: Message):
    student_id = 1
    lesson_id = 1
    ...
    try:
        ...
        if response.status_code == 201:
            await message.answer("✅ Посещаемость успешно отмечена!")
        else:
            await message.answer("❌ Ошибка при отметке: " + response.text)
...
async def show_report(callback: CallbackQuery):
    ...
    response = requests.get(f"http://127.0.0.1:8000/api/report/{student_id}/")
    data = response.json()
    report_text = (
        f"📊 Отчёт по ученику:\n"
        f"✅ Посещено: {data['Посещено']} дней\n"
        f"❌ Пропущено: {data['Пропущено']} дней\n"
        f"📆 Всего занятий: {data['Всего']}"
    )
